...
class nlpProb(object):
    ...
    def __init__(self, N, T, t0, x0, ncons, nu, lanes, obstacle):
        self.N = N
        self.T = T
        self.t0 = t0
        self.x0 = x0
    ...
    def objective(self, u):
        ...
        for k in range(N):
            ...
            #uk = np.array([u[k]])
            costvec[k] = prob.runningCosts( uk, x[k], t0 + k*T, lanes, obstacle)
            cost = cost + costvec[k]
    ...
    def constraints(self, u):
        ...
        for k in range(N):
            consR1_R[k], consR1_L[k] = prob.runningCons(u, x[k], t0, lanes, obstacle)
        ...
        if nstates == 6:
            ...
            # terminal constraint
            consT1, consT2 = prob.terminalCons(u, x[N - 1], t0, lanes, obstacle)
            ...
            # terminal constraint
            consT1, consT2 = prob.terminalCons(u, x[N-1], t0, lanes, obstacle)  # ydist, VEnd
            consT = consT1
    ...
    def setup(self, u0):
        ...
        nlp = ipopt.problem(
            n=nu*N,
            m=len(cl),
            problem_obj=nlpProb(N, T, t0, x0, ncons, nu, lanes, obstacle),
            lb=lb,
            ub=ub,
            cl=cl,
            cu=cu
